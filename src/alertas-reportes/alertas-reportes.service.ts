...
export class AlertasReportesService {
  ...
  async generarReporte(idPaciente: string) {
    try {
      ...
      const report = this.buildTimeSeriesReport(
        sesiones as SesionPuntajeDto[],
        idPaciente,
      );
      return report;
    } catch (error) {
  ...
  private buildTimeSeriesReport(
    sesiones: SesionPuntajeDto[],
    patientId?: string,
  ) {
    ...
    const get = (key: string) =>
      sessions.map((s) => (s[key] !== undefined ? s[key] : null));
    ...
    if (
      count >= 2 &&
      typeof timeseries.sessionTotal[0] === 'number' &&
      typeof timeseries.sessionTotal[count - 1] === 'number'
    ) {
      ...
      trend_sessionTotal =
        slopePerDay_sessionTotal > 0.005
          ? 'increasing'
          : slopePerDay_sessionTotal < -0.005
            ? 'decreasing'
            : 'stable';
